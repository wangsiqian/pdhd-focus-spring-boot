...
---
[ai-helper.md](../../../../Desktop/ai-helper.md)
## 1. 产品背景与目标

### 1.1 背景

传统日历（Google Calendar/Outlook）只记录“计划”，无法真实反映“时间去哪了”。对于 ADHD 人群，最大的痛点在于：
...
- **返回字段**: 所有实体都会默认字段，但是在返回实体参数时，如果未做特别声明，只返回`create_at`参数，并倒序排序
- **请求参数**: 在根据实体生成请求参数时，请不要携带默认字段，以及用户信息（从上下文取），请求参数命名规则 {方法名驼峰}Req.java，不允许复用多个接口的 Req 实体
- **禁止使用**: `@PathVariable`
- **方法选择**: 根据不同的方法选择GET、POST请求
    - 对于GET方法，参数如果超过3个，或者包含复杂、LIST/SET 类型等参数，需要改成POST方法
    - POST方法统一使用 @RequestBody 接收参数
- **统一接口**: 更新和创建统一为`/upsert`接口，service 层也统一为 upsert 方法，通过判断参数里面 id 是否为null，来决定更新还是创建
- **参数验证**:
  - 请需要通过 @Valid 注解，验证一些参数的合法性，保证代码的鲁棒性

### 代码规范
- **日志**: 
  - 只添加关键步骤日志，保证日志的可追溯性、代码的可维护性，但是要注意日志内容不要超长，和保留关键信息
  - 日志内容描述用中文
- **设计原则**: 遵循单一职责、DRY等规范
- **可维护性**: 保证代码的可读性、可维护性
- **通用性**: 尽量保障代码的通用性，如：详情、列表、分页、不同场景下的列表等接口，对应的虽然是多个 controller 接口，但是底层共用同一个 service 方法
- **框架使用**: 
  - 数据查询使用 mybatis-plus 的 lambda 链式表达式
  - 必须使用 hutool 和 google common 来做对象判断
  - 必须使用 google common 来初始化集合对象

### 对内提供接口
`xxx-api` 目录是对内部提供的 openfeign 接口

- 后续都在 req 目录存放接口的请求参数
- 后续都在 resp 目录存放接口的返回参数
- 后续都在根目录存放 openfeign 接口，类名尽量按实体 xxxApi 命名
- 对内提供的接口请求路径以 `/api/v1/{实体名复数形式}` 开头

### 对外提供接口
对C端提供的接口以`/webApi/{实体名复数形式}`开头

### 项目规范
- ContextUtils 获取登录人信息